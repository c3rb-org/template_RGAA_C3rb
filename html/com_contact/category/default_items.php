...
<?php else : ?>
	...
	<ul class="category list-striped list-group"><!-- modif tmpl -->
			<?php foreach ($this->items as $i => $item) : ?>
				<?php if (in_array($item->access, $this->user->getAuthorisedViewLevels())) : ?>
					<?php if ($this->items[$i]->published == 0) : ?>
						<li class="system-unpublished cat-list-row<?php echo $i % 2; ?> list-group-item"><!-- modif class tmpl -->
					<?php else: ?>
						<li class="cat-list-row<?php echo $i % 2; ?> list-group-item" ><!-- modif class tmpl -->
					<?php endif; ?>
		<?php if ($this->params->get('show_image_heading')) : ?>
					...
					<div class="clearfix"></div><!-- Ajout tmpl -->
					</li>
				<?php endif; ?>
			<?php endforeach; ?>
		</ul>
