...
abstract class JHtmlJGrid
{
                                  ...
                                  $inactive_class = '', $enabled = true, $translate = true, $checkbox = 'cb')
    {
        ...
        if ($enabled)
        {
            $html[] = '<a class="btn btn-default btn-xs' . ($active_class == 'publish' ? ' active' : '') . ($tip ? ' hasTooltip' : '') . '"';
        ...
        {
            $html[] = '<a class="btn btn-default btn-xs disabled jgrid' . ($tip ? ' hasTooltip' : '') . '"';
            $html[] = $tip ? ' title="' . $title . '"' : '';
            $html[] = '>';
    ...
    public static function published($value, $i, $prefix = '', $enabled = true, $checkbox = 'cb', $publish_up = null, $publish_down = null)
    {
        ...
        $states = array(1 => array('unpublish', 'JPUBLISHED', 'JLIB_HTML_UNPUBLISH_ITEM', 'JPUBLISHED', true, 'check', 'publish'),
            0 => array('publish', 'JUNPUBLISHED', 'JLIB_HTML_PUBLISH_ITEM', 'JUNPUBLISHED', true, 'times-circle', 'unpublish'),
